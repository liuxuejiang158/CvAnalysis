...
def cal_dimensions():
    conn=MongoClient("192.168.4.249",27017)['combine']['combine_info']#修改源数据库地址
    write_location=MongoClient("192.168.4.249",27017)['shulianxunying']['combine_info_dimension']#修改目的数据库地址
    max_salary=300000.
    '''
    print("计算最大期望薪金")
...
if __name__ =="__main__":
    if len(sys.argv)<2:
        print("Input args:cal_combine_dimension")
        sys.exit(1)
    elif sys.argv[1]=="cal_combine_dimension":
        #read_academic_level()
        #read_job_exp()
        read_chacater_word()
        read_skill_weight()
